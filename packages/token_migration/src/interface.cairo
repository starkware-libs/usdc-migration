...
pub trait ITokenMigrationAdmin<T> {
    /// Sets the token supplier address.
    /// Must be called during initialization process, swaps cannot proceed until set.
    /// Precondition: L1 recipient is verified.
    /// Caller must be the owner.
    fn set_token_supplier(ref self: T, token_supplier: ContractAddress);
...
pub trait ITokenMigrationStarkGate<T> {
    ...
    fn get_new_token_supply(self: @T) -> u256;
}
