...
fn end_to_end_swap_send_to_l1_test() {
    ...
    let events = spy.get_events().emitted_by(contract_address: legacy.contract_address).events;
    // 1 for approval
    // + 1 for transfer from user to contract
    // + 1 for approval change
    // + amount / LARGE_BATCH_SIZE for l1 transfers
    assert_number_of_events(
        actual: events.len(),
        expected: 3 + (amount / LARGE_BATCH_SIZE).try_into().unwrap(),
        message: "end_to_end_swap_send_to_l1_test",
    );
