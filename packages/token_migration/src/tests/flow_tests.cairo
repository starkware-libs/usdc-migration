...
fn test_set_legacy_buffer_flow() {
    ...
    // Swap that does not trigger send to l1 (would have without the privious
    // set_legacy_buffer).
    let amount = 2;
    let user_1 = new_user(id: 1, token: cfg.legacy_token, initial_balance: amount);
    approve_and_swap_to_new(:cfg, user: user_1, :amount);
    ...
    // Swap that trigger send to l1 (would not have without the privious set_legacy_buffer).
    let amount = 1;
    let user_2 = new_user(id: 2, token: cfg.legacy_token, initial_balance: amount);
    approve_and_swap_to_new(:cfg, user: user_2, :amount);
    assert_balances(
...
fn test_set_batch_size_flow() {
    ...
    // Swap that trigger send to l1 (would not have without the privious set_batch_size).
    let amount = 1;
    let user_1 = new_user(id: 1, token: cfg.legacy_token, initial_balance: amount);
    approve_and_swap_to_new(:cfg, user: user_1, :amount);
    assert_balances(
