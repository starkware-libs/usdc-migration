use constants::{
    DECIMALS, INITIAL_SUPPLY, L1_RECIPIENT, L1_TOKEN_ADDRESS, LEGACY_BUFFER, OWNER_ADDRESS,
...
pub(crate) fn deploy_tokens(owner: ContractAddress) -> (Token, Token) {
    let legacy_config = TokenConfig {
        name: "Token", symbol: "Token", decimals: DECIMALS, initial_supply: TOKEN_SUPPLY, owner,
    };
