...
pub mod USDCMigration {
    ...
    pub impl USDCMigrationImpl of IUSDCMigration<ContractState> { //impl logic
        ...
        fn swap_to_legacy(ref self: ContractState, amount: u256) {
            self
                ._swap(
                    from_token_dispatcher: self.new_token_dispatcher.read(),
                    to_token_dispatcher: self.legacy_token_dispatcher.read(),
                    :amount,
                );
        }
    }
    ...
    impl USDCMigrationInternalImpl of USDCMigrationInternalTrait {
        fn _swap(
            ...
            self
                .emit(
                    USDCMigrated {
                        user: caller_address, to_token: to_token.contract_address, amount,
                    },
                );
        }
    }
